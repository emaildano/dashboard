<project_detail>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper" class="gray-bg">
            ...
            <div id="container_progress" if="{ status !== 'error' && status === 'inimport'  }" class="row hidden" style="margin-top: 20px;">
                <div class="col-lg-12">
                    <div class="alert alert-success alert-dismissable"  >
                        <h4>{wp_status_msg[apps.step].title}</h4>
                        <p>{wp_status_msg[apps.step].message}</p>
                        <dl if="{ status === 'inimport' }" class="dl-horizontal" style="margin-top:30px;">
                            <dt>Progress:</dt>
                            <dd>
                                <div class="progress progress-striped active m-b-sm">
                                    <div style="width: {wp_status_msg[apps.step].progress}%;" class="progress-bar"></div>
                                </div>
                            </dd>
                        </dl>
                    </div>
            ...
            <div id="generate_progress" if="{ status === 'generating' }" class="row" style="margin-top: 20px;">
                <div class="col-lg-2">
                  <div class="ibox">
                    <div class="ibox-content">
                       <h5>Generated pages</h5>
                       <h1 class="text-center">{gen.created_url || '-'} / {gen.sum_url || '-'}</h1>
                    </div>
                  </div>
                </div>
                <div class="col-lg-10">
                    <div class="alert alert-success alert-dismissable"  >
                        <h4>{gen_status_msg[gen.step].title}</h4>
                        <p>{gen_status_msg[gen.step].message}</p>
            ...
            <div if="{ status === 'error' }" class="row" style="margin-top: 20px;">
                <div class="col-lg-12">
                        ...
                        <div class="ibox">
                            <div class="ibox-content">
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="m-b-md">
                                            <a href="/#projects" class="btn btn-white btn-xs pull-right">Back to Project</a>
                                        </div>
                                        <dl class="dl-horizontal dl-top">
                                            <dt>Name:</dt> <dd><a href="#" id="project_name" data-type="text" data-pk="{ site_id }" data-url="{project_endpoint +  site_id }" data-title="Enter { project_name }">{ project_name }</a></dd>
                                            <dt>ID:</dt> <dd>{ site_id }</dd>
                                            <dt>Disk Usage:</dt> <dd>{ disk_usage }</dd>
                                            <dt>Status:</dt>
                                            <dd>
                                                <span if="{ status === 'generating' }" id="site_processing" class="label label-info">Generating</span>
                                                <span if="{ status === 'starting' || status === 'start' }" id="app_running" class="label label-info">running</span>
                                                <span if="{ status === 'stop' }" id="app_stopped" class="label label-warning">stopped</span>
                                                <span if="{ status === 'inimport' }" id="app_importing" class="label label-warning">importing</span>
                                            </dd>
                                        </dl>
                                    </div>
                                </div>
                                  ...
                                  <div class="col-lg-12">
                                    ...
                                    <div class="tabs-container">
                                        <ul class="nav nav-tabs">
                                            ...
                                            <li class=""><a data-toggle="tab" href="#netlify" aria-expanded="false"> Netlify</a></li>
                                        </ul>
                                        <div class="tab-content">
                                            <div id="general" class="tab-pane active">
                                                <div class="panel-body">
                                                    <dl class="dl-horizontal">
                                                        <dt>Domain:</dt>
                                                        <dd><a href="https://{ domain_name }" target="_blank" class="{ domain_name ? 'link-enabled' : 'link-disabled' } ">{ domain_name }</a><button if="{ !domain_name }" id="edit" class="btn btn-white btn-xs m-l-sm"  type="button" data-toggle="modal" data-target="#attach-domain" disabled="{ disabled: validDomains.length <= 0 }">Attach Domain</button><button if="{ domain_name }" id="edit" class="btn btn-white btn-xs m-l-sm"  type="button" onclick="{ dettachDomain }">Dettach Domain</button></dd>
                                                        <dt>Published URL:</dt>
                                                        <dd if="{ (histories.length > 0 || status === 'generating') && latest_gen_history !== 'static_delete'  }"><a href="https://{ access_url }" target="__blank">{ access_url }</a></dd><dd if="{ histories.length <= 0 && status !== 'generating' || latest_gen_history === 'static_delete' }">-</dd>
                                                        <dt>Published Date:</dt> <dd>{ update_time }</dd>
                                                    </dl>
                                                    <div class="row">
                                                      <div class="col-lg-12">
                                                        <dl class="dl-horizontal" if="{ status !== 'generating' }">
                                                            <dt></dt>
                                                            <dd>
                                                              <span if="{maintenance.status === 'now_maintenance' && ( maintenance.target.indexOf('generator') >= 0 || maintenance.target.indexOf('container') >= 0 )}">Sorry. Now maintenance. Generate can&apos;t be used</span><br />
                                                              <button id="generate_publish_site" class="ladda-button ladda-button-demo btn btn-success " type="button" data-style="expand-right" onclick="{ create_staticsite }" disabled="{ disabled: status === 'starting' || status === 'intasks' || status === 'inimport' || status === 'start' || (maintenance.status === 'now_maintenance' && ( maintenance.target.indexOf('generator') >= 0 || maintenance.target.indexOf('container') >= 0)) }">
                                                                <i class="fa fa-cloud-upload"></i>&nbsp;&nbsp;
                                                                <span class="bold">Generate Now</span>
                                                              </button>
                                                              <button id="delete_publish_site" class="ladda-button ladda-button-demo btn btn-warning pull-right" type="button" data-style="expand-right" onclick="{ delete_staticsite }" disabled="{ disabled: maintenance.status === 'now_maintenance' && maintenance.target.indexOf('container') >= 0 }">
                                                                <i class="fa fa-trash"></i>&nbsp;&nbsp;
                                                                <span class="bold">Delete All Files from Your Published Site</span>
                                                              </button>
                                                            </dd>
                                                        </dl>
                                                      </div>
                                            ...
                                            <div id="netlify" class="tab-pane">
                                                <netlify
                                                    siteid = { site_id }
                                                    status = { status }
                                                    last = { latest_gen_history }
                                                    histories = { histories }
                                                  />
                                            </div>
                                        </div>
                                ...
                                <div class="row">
                                  <div class="col-lg-12">
                                    ...
                                    <dl class="dl-horizontal">
                                        <dt class="apps_url">App URL:</dt>
                                        <dd if="{ status !== 'stop' && status !== 'generating' && status !== 'intasks' && maintenance.status !== 'now_maintenance' }" class="apps_url">
                                          <a href="{docker_url}" target="_blank">{ docker_url }</a>
                                        </dd>
                                        <dd if="{ status === 'stop' || status === 'generating' || status === 'intasks' || maintenance.status === 'now_maintenance' }" class="apps_url">
                                          -
                                        </dd>
                                        ...
                                        <dd>
                                          <div class="col-lg-2">
                                            <select id="php_version" class="form-control m-b" name="php_version" >
                                              <option value="5.5" selected="{ selected: php_version == 5.5 }">5.5</option>
                                              <option value="5.6" selected="{ selected: php_version == 5.6 }">5.6</option>
                                              <option value="7.0" selected="{ selected: php_version == 7.0 }">7.0</option>
                                            </select>
                                          </div>
                                        </dd>
                                    </dl>
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <dl class="dl-horizontal" if="{ status !== 'generating' }">
                                        <dt></dt>
                                        <dd>
                                          <span if="{maintenance.status === 'now_maintenance' && maintenance.target.indexOf('container') >= 0 }">Sorry. Now maintenance. App can&apos;t be used</span><br />
                                          <button if="{ status === 'stop' || status === 'starting' || status === 'intasks' }" id="start_app" class="ladda-button ladda-button-demo btn btn-success" type="button" data-style="expand-right" onclick="{ create_container }" disabled="{ disabled: maintenance.status === 'now_maintenance' && maintenance.target.indexOf('container') >= 0 || status === 'intasks' }">
                                            <i class="fa fa-play-circle"></i>&nbsp;&nbsp;
                                            <span class="bold">Start App</span>
                                          </button>
                                          <button if="{ status === 'start' || status === 'error' }" id="stop_app" class="ladda-button ladda-button-demo btn btn-warning" type="button" data-style="expand-right" onclick="{ delete_container }" disabled="{ disabled: maintenance.status === 'now_maintenance' && maintenance.target.indexOf('container') >= 0 }">
                                            <i class="fa fa-stop"></i>&nbsp;&nbsp;
                                            <span class="bold">Stop App</span>
                                          </button>
                                          <button if="{ status !== 'inimport' }" id="createArchive" class="ladda-button ladda-button-demo btn btn-warning" type="button" data-style="expand-right" onclick="{ createArchive }" disabled="{ disabled: maintenance.status === 'now_maintenance' && maintenance.target.indexOf('container') >= 0 || status === 'start' || status === 'starting' }">
                                            <i class="fa fa-file-zip-o"></i>&nbsp;&nbsp;
                                            <span class="bold">Export as a new archive</span>
                                          </button>
                                        </dd>
                                    </dl>
                                </div>
                                        ...
                                        <div class="panel blank-panel">
                                            <div class="panel-heading">
                                                <div class="panel-options">
                                                    <ul class="nav nav-tabs">
                                                        <li class="active"><a href="#tab-1" data-toggle="tab">Generate History</a></li>
                                                    </ul>
                                                </div>
                                            </div>
                                                ...
                                                <div class="tab-content">
                                                    <div class="tab-pane active" id="tab-1">
                                                        <table class="table table-striped">
                                                            ...
                                                            <tbody>
                                                                <tr each={ h, i in histories }>
                                                                    <td>
                                                                        { username }
                                                                    </td>
                                                                    <td>
                                                                        { moment(h.update_time).format("MMM Do YYYY, h:mm:ss a") }
                                                                    </td>
                                                                    <td>
                                                                        { h.progress }
                                                                    </td>
                                                                    <td>
                                                                        { h.progress === 'fail' ? h.message : '' }
                                                                    </td>
                                                                </tr>
                                                            </tbody>
                                                        </table>
    ...
    <div class="modal inmodal" id="attach-domain" tabindex="-1" role="dialog" aria-hidden="true">
     <div class="modal-dialog">
       <div class="modal-content animated bounceInRight">
         <div class="modal-header">
           <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
           <h4 class="modal-title">Attach Domain</h4>
         </div>
         <div class="modal-body">
           <p>Choose the domain to attach your published site on the project. <br/>After assigning domain, you should configure your DNS server to access published site with your domain.<br/>e.g. Set Published URL to the CNAME record  for your domain;<br/>Set Published URL to the CNAME record of your domain with www prefix (www.example.com); <br/>Set Published URL to the A record (or alias record) of your domain.<br />Attaching domain takes 10-15 minutes to complete.</p>
           <h3>Notice!</h3>
           <p>Assigning Zone Apex to Shifter, you should use DNS service who supports it. <br/>
              <a href="https://getshifter.zendesk.com/hc/en-us/articles/115001014908" target="_blank">See more detail</a>
            </p>
           <div class="form-group">
             <label>Domain Name</label>
             <select class="form-control m-b" id="attachedDomains">
               <option each={ h, i in validDomains } value="{h}">{h}</option>
             </select>
           </div>
         </div>
         <div class="modal-footer">
           <button type="button" class="btn btn-white" data-dismiss="modal">Close</button>
           <button id="attachDomain" type="button" class="btn btn-primary ladda-button ladda-button-demo" data-style="expand-right" onclick="{ attachDomain }">Attach</button>
         </div>
       </div>
      </div>
    </div>
